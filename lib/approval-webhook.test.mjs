...
const approval = await import("file:///home/runner/work/wex2026/wex2026/lib/approval-webhook.ts")

test("approval requests get a cleanup token and redirect on timeout", () => {
  const request = approval.createApprovalRequest("timeout-user", "password", "approval-timeout-test")

  assert.equal(typeof request.cleanupToken, "string")
  assert.notEqual(request.cleanupToken.length, 0)
